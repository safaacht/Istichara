<?php
session_start();
require_once __DIR__ . '/../repositories/VilleRepo.php';
...
<body>
        ...
        <form action="index.php?controller=demande&action=Registerpro" method="POST" enctype="multipart/form-data" id="multiStepForm">
            ...
            <div class="form-step" data-step="3">
                ...
                <div style="background: #fff3cd; border: 1px solid #ffc107; border-radius: 8px; padding: 15px; margin-bottom: 20px;">
                    <strong>⚠️ Note importante:</strong> Votre compte sera mis en attente de validation par un administrateur. Vous recevrez un email une fois votre compte approuvé.
                </div>
    ...
    <!-- <script>
        ...
        document.querySelectorAll('input[name="type"]').forEach(radio => {
            radio.addEventListener('change', function() {
                ...
                if (this.value === 'avocat') {
                    ...
                    document.getElementById('type_actes').required = false;
                } else {
                    avocatFields.style.display = 'none';
                    huissierFields.style.display = 'block';
                    document.getElementById('specialization').required = false;
                    document.getElementById('type_actes').required = true;
                }
            });
        });

        // File upload display
        document.getElementById('carte_pro').addEventListener('change', function() {
            document.getElementById('carte_pro_name').textContent = this.files[0] ? this.files[0].name : '';
        });

        document.getElementById('diplomes').addEventListener('change', function() {
            const count = this.files.length;
            document.getElementById('diplomes_name').textContent = count ? `${count} fichier(s) sélectionné(s)` : '';
        });
        ...
        function populateReview() {
            ...
            document.getElementById('review_type').textContent = type === 'avocat' ? 'Avocat' : 'Huissier';
            ...
            document.getElementById('review_city').textContent = citySelect.options[citySelect.selectedIndex].text;
            
            document.getElementById('review_experience').textContent = document.getElementById('experience').value + ' ans';
            document.getElementById('review_tarif').textContent = document.getElementById('tarif').value + ' MAD';
            
            const cartePro = document.getElementById('carte_pro').files[0];
            document.getElementById('review_carte').textContent = cartePro ? cartePro.name : '';
            
            const diplomes = document.getElementById('diplomes').files;
            document.getElementById('review_diplomes').textContent = diplomes.length + ' fichier(s)';
        }
